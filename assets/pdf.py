#!/usr/bin/env python3
"""
pdf.py — the single PDF renderer for every Productivity skill.
...
Usage
-----
    python3 pdf.py --kind daily_brief --in brief.json --out Daily_Brief_2026-09-09.pdf
    python3 pdf.py --kind call_prep   --in call.json  --out 1200-acme.pdf
    python3 pdf.py --batch manifest.json        # many PDFs, one process
...
def render_follow_ups(data, path):
    """DEPRECATED — follow-ups no longer renders or attaches anything.
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Render a Productivity skill PDF.")
