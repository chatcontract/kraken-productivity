...
**Invoke this skill as `/kraken-productivity:kraken-call-prep`.** Plain English reaches it too (see the description above); the namespaced command is what to use in a runbook or when it matters that the right skill runs first time.
...
- **Scheduled: weekday mornings, Monday to Friday, 04:00 local, prepping TODAY** — `kraken-onboarding` owns the cron and is the only thing that writes it. Four o'clock gives every call of the day, including an 8 AM one, a booked block well before anyone is awake to need it.
...
| Field | Value |
|---|---|
| **Title** | `Kraken Call Prep — settings` |
...
**Find it by title search, not by date** — a `search_events` for `Kraken Call Prep — settings`, fired in Step 1's batch. It is not on today's calendar, so the day's `list_events` will never return it.
...
2. One `search_events` for the title `Kraken Call Prep — settings` — the settings record, which holds the scope preference, the lead time, and the opt-out and booked lists. **It is dated to the first run, so the day's `list_events` will never return it**; it has to be searched for by title.
...
Full briefing → <link>
ref: kraken-call-prep/5gglp4va2edl3cco4qpkd7nqjh
```
...
Where the event carries a **recurring-series ID**, the line carries both: `ref: kraken-call-prep/<series id>/<occurrence id>`. Capture the series ID in Step 1 alongside start and end; the series half is what an opt-out is recorded against.
...
- The call's own description contains `no prep` or `kraken: skip`.
