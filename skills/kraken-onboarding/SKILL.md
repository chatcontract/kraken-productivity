...
description: Schedules the Kraken Productivity weekday skills — Call Prep 4 AM, Daily Brief 6 AM, Follow-Ups 4 PM, Monday to Friday. All three, or one when a skill is named. Runs the mandatory-connector gate first, resolves skill names whatever the namespace, deletes legacy tasks so nothing double-sends, then asks you to set Permissions to Automatically approve. Creates tasks only. Use /kraken-productivity:kraken-onboarding, "onboard me", or after a plugin update.
---
...
|---|---|---|
| **Full** (default) | `/kraken-productivity:kraken-onboarding`, "onboard me", "set up my Kraken skills", or after a plugin update | All three tasks. |
...
| Skill | Command |
|---|---|
| Onboarding — run this first | `/kraken-productivity:kraken-onboarding` |
...
|---|-------|---------|-----------|------|
| 1 | `kraken-call-prep` | Weekdays Mon–Fri | 4:00 AM | `0 4 * * 1-5` |
| 2 | `kraken-daily-brief` | Weekdays Mon–Fri | 6:00 AM | `0 6 * * 1-5` |
| 3 | `kraken-follow-ups` | Weekdays Mon–Fri | 4:00 PM | `0 16 * * 1-5` |
...
> `Kraken Productivity v<VERSION> — prompt generated by kraken-onboarding. If this line names an older version than the installed plugin, re-run kraken-onboarding to refresh.`
...
Use the same three task IDs every time — `kraken-daily-brief`, `kraken-follow-ups`, `kraken-call-prep` — and **never invent versioned IDs** like `kraken-daily-brief-v2`, which would leave the old task running alongside the new one and double every email.
...
- **Resolve the named skill** per *Resolving skill names*, so `daily brief`, `kraken-daily-brief` and `productivity:kraken-daily-brief` all land on the same target.
...
This plugin ships under four names — Kraken, Haynes, AllNeurons and Roots — and earlier releases used others. A user who onboarded under any of them has scheduled tasks whose IDs this skill no longer writes to, so they would keep firing **alongside** the new ones and the user would get two of every email. **Kraken is this package; every other prefix is legacy here.**
...
|---|---|
| `haynes-daily-brief`, `allneurons-daily-brief`, `roots-daily-brief`, `an-daily-brief` | `kraken-daily-brief` |
| `haynes-follow-ups`, `haynes-follow-up`, `allneurons-follow-ups`, `allneurons-follow-up`, `roots-follow-ups`, `roots-follow-up`, `an-follow-ups`, `an-follow-up` | `kraken-follow-ups` |
| `haynes-call-prep`, `allneurons-call-prep`, `roots-call-prep`, `an-call-prep` | `kraken-call-prep` |
...
The three may be installed standalone (`kraken-daily-brief`) or inside a plugin (`<plugin>:<skill>`).

**The namespace is never matched against — it is discarded before comparison.** Whatever the plugin is called and however spelled or capitalized — `kraken-productivity`, the same name in any casing, a misspelling of it, `anthropic-skills`, anything at all — only the part after the final `:` is compared. Never hardcode, guess, or ask about the namespace.

**Comparison is normalized:** lowercase, then strip every non-alphanumeric character. So `kraken-daily-brief`, `kraken_daily_brief`, and that same name capitalised or punctuated any way at all, every one normalizes to `krakendailybrief`.
...
3. Same two checks after stripping a leading brand segment — this package's own `kraken-`, or any legacy prefix in the *Migrating from an earlier install* table above — from **both** sides before normalizing. This covers `productivity:daily-brief` and keeps earlier installs resolving to the same skill. Strip the hyphenated segment, never bare letters from the normalized string.
...
Ties at the same tier: prefer a namespace normalizing to `krakenproductivity` or `productivity`; otherwise take the first. Record the resolved name verbatim.
...
|---|---|
| A person — `/kraken-productivity:kraken-onboarding`, "onboard me", "schedule my follow-ups" | **Run it in full**, per Steps 2 and 3. |
...
| Task ID | Title (exact) |
|---|---|
| `kraken-daily-brief` | `Kraken Daily Brief` |
| `kraken-follow-ups` | `Kraken Follow-Ups` |
| `kraken-call-prep` | `Kraken Call Prep` |
...
|---|---|---|---|---|---|
| `kraken-daily-brief` | `0 6 * * 1-5` | `krakendailybrief` | `dailybrief` | `MODE = today's brief` | this is a scheduled unattended run, so it is Monday to Friday only |
| `kraken-follow-ups` | `0 16 * * 1-5` | `krakenfollowups` | `followup` | `MODE = full sweep, drafts only — never send a follow-up` | this is a scheduled unattended run, so it is Monday to Friday only |
| `kraken-call-prep` | `0 4 * * 1-5` | `krakencallprep` | `callprep` | `MODE = prep TODAY's external calls; book a prep block per call, do not email unless the run fails` | this is a scheduled unattended run, so it is Monday to Friday only — **today's weekday is the only check**, because a 4 AM run preps today |

**Descriptions** (suffix each with ` · v<VERSION>`):

- `Kraken Daily Brief` — Weekday 6 AM briefing: today's meetings, conflicts, what's due, open asks, emailed as one PDF.
- `Kraken Follow-Ups` — Weekday 4 PM sweep: finds who needs a follow-up, drafts replies in your voice, never sends.
- `Kraken Call Prep` — Weekday 4 AM prep: a private prep block before each external call today, with the dossier linked.
...
>
> To re-run this setup at any time — after a plugin update, or to clear duplicate tasks — use `/kraken-productivity:kraken-onboarding`.
...
**Exact strings — never alter these.** The three task titles are `Kraken Daily Brief`, `Kraken Follow-Ups` and `Kraken Call Prep` — that capitalisation exactly, including the hyphen and capital **U** in **Follow-Ups**. The sidebar section is **Scheduled**, not "Schedule" or "Schedules". The button is **Edit**, clicked **twice** — once on the task, once again to reach the schedule settings. The setting is **Permissions**, its values are **Manually approve** and **Automatically approve**, and the confirm button is **Save**. If you find yourself writing "Automatic Approval", "Auto-approve", "Settings" or "Kraken Follow-ups", you have got it wrong — go back to the strings above.
